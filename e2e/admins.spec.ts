...
test.describe("Admin Panel - CRUD state verification", () => {
  // Run every test in this file one at a time, across both projects.
  // The admin app runs against a Next.js DEV server here, which compiles
  // routes on demand - concurrent workers hitting it at once can stall
  // save/delete round-trips long enough to time out unrelated assertions
  // (dialogs not closing, "Saving..." never disappearing). Serial mode
  // removes that contention; if flakiness persists after this, the issue
  // isn't concurrency and Fix 3 (test against a prod build) is the next step.
